...
#define ERR_HEAD "*** DEL2[mex]: "
#define ERR_ID   "WHEN:DEL2:"
                    ...
                    double *Y)
{
  ...
  for(int x = 1; x < row -1; x ++)
  	for(int y = 1; y < col - 1; y++)
  		Y[x * col + y] = (X[(x - 1) * col + y] + X[(x + 1) * col + y] + X[x * col + y + 1] + X[x * col + y - 1] - 4 * X[x * col + y]) / 4;

  //edge points
  //x = 0
  int x = 0;
  for(int y = 1; y < col - 1; y++)
  	Y[x * col + y] = (-5 * X[(x + 1) * col + y] + 4 * X[(x + 2) * col + y] - X[(x + 3) * col + y] + X[x * col + y + 1] + X[x * col + y - 1]) /4;
  ...
  int y = 0;
  for(int x = 1; x < row - 1; x++)
  	Y[x * col + y] = (-5 * X[x * col + y + 1] + 4 * X[x * col + y + 2] - X[x * col + y + 3] + X[(x + 1) * col + y] + X[(x - 1) * col + y]) /4;
  ...
  x = 0;
  y = 0;
  Y[x * col + y] = (-5 * X[x * col + y + 1] + 4 * X[x * col + y + 2] - X[x *col + y + 3] - 
  	5 * X[(x + 1) *col + y] + 4 * X[(x + 2) *col + y] - X[(x + 3) * col + y] + 4 * X[x * col + y]) / 4;
