function phi = drlse2d(phi_0, g, lambda,mu, alfa, epsilon, timestep, iter, potentialFunction)
...
for k=1:iter
    ...
    phi_x = DGradient(phi, 1, 2);
    phi_y = DGradient(phi, 1, 1);
    ...
    elseif strcmp(potentialFunction,'double-well');
        distRegTerm=distReg_p2(phi, phi_x, phi_y);  % compute the distance regularization term in eqaution (13) with the double-well potential p2.
    else
...
function f = distReg_p2(phi, phi_x, phi_y)
...
nxx = DGradient(nx, 1, 2);  
nyy = DGradient(ny, 1, 1);
f = nxx+nyy;
